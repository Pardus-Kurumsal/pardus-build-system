...
spawn_container() {
    ...
    docker run -de POSTGRES_PASSWORD=${postgres_passwd} --name=postgressetup \
        -v ${docker_vols}/postgres:/var/lib/postgresql/data kiasaki/alpine-postgres:9.5 > /dev/null 2>&1
...
create_db_user distrotracker ${DTRACKER_PASSWD}

spawn_container ${POSTGRES_PASSWD} ${DOCKER_VOLS} ${POSTGRES_DELAY}

do_psql ./bootstrap.sql
create_db gogs gogs
create_db distrotracker distrotracker
